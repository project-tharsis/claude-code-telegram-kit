...
For every user-facing Telegram response, call `mcp__telegram-renderer__send_reply` exactly once with the inbound `chat_id`, optional `reply_to`, and one unescaped CommonMark/GFM document in `content`.
