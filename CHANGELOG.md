...
- Progress disclosure no longer folds after eight lines: it keeps every normal parent and subagent tool step visible, uses separate-line inline code for sanitized previews without Telegram's obstructive code-block chrome, names `Skill` calls explicitly, and reserves folding for the 4096-character wire limit.
