...
- Interim commentary capture now uses Claude Code's native bounded `MessageDisplay` stream. Completed messages are buffered in memory and committed by a later PreToolUse; the commentary rail no longer reads JSONL.
