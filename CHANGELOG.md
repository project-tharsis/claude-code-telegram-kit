...
- Root-asset upgrades now preserve an existing secure deployment-specific `activation.json` instead of overwriting it with the generic example; first installs still bootstrap the rendered template, unsafe existing files fail closed, the manifest records the exact preserved bytes, and post-read ownership/mode/link metadata is revalidated before acceptance.
- The isolated Memory reviewer now uses low Claude effort, a two-turn ceiling for structured-output completion, and a bounded 75-second generation timeout, leaving margin under the root worker's 90-second process boundary for real review prompts that exceeded the previous one-turn/45-second limits.
...
- Claude runtime failures now produce one quoted deterministic notice instead of silent Channel stalls. Typed `StopFailure.error` is primary; because Claude Code 2.1.241 can surface background quota exhaustion only as a failed task notification plus a structured `isApiErrorMessage` transcript row, the exact consumed task route also gets a five-second bounded append watcher. Notices use only the fixed error enum and validated `quotaLimits.resetsAt`, dedupe per chat/error incident, never ingest provider prose, and never retry an unknown Telegram outcome. When a typed rate-limit event lacks reset metadata, the renderer may enrich it from the private, secure `statusLine.rate_limits` snapshot only if at least one subscription window is exactly exhausted; multiple exhausted windows use the latest reset, and stale, unsafe, expired, or non-exhausted snapshots keep the generic notice. Failed notifications also clear automatic-title task deferral.
