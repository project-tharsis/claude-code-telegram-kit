...
8. `/usage` remains available while Claude is blocked by auth or a quota modal: the activation-attested control sidecar consumes only fresh exact `/usage` enqueue appends from startup-pinned transcript FDs and renders opt-in OAuth realtime percentages when available, otherwise only a statusLine snapshot no more than 15 minutes old with its capture time; active quota renders reached/reset without bars, all without an extra Claude process, model turn, or second Telegram poller; while structured quota state is active, later ordinary messages receive the same quoted reset notice instead of silent `👀`;
...
19. a forked `Skill`/agent sends one quoted `Background work` bubble after the parent final; nested tool and `SubagentStop` events edit it without any timer or model check-in, and a later direct prompt does not retire it while the agent remains active. Completion then arrives without another user message, quotes the original Telegram request, and emits no third progress bubble, typing heartbeat, artifact pass, or reaction change. If that authority-bound completion turn forks a downstream agent, its exact tool routes repeat the same bounded contract for one descendant phase; missing, expired, and replayed routes remain silent, and automatic title state remains unclaimed until completion.
