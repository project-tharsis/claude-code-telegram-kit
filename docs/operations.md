...
8. `/usage` remains available while Claude is blocked by auth or a quota modal: the activation-attested control sidecar consumes only fresh exact `/usage` enqueue appends from startup-pinned transcript FDs and renders reached/reset only unless the opt-in on-demand OAuth fetch returns realtime percentages, without an extra Claude process, model turn, or second Telegram poller; while structured quota state is active, later ordinary messages receive the same quoted reset notice instead of silent `👀`;
