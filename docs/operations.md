...
8. `/usage` remains available while Claude is blocked by auth or a quota modal: the activation-attested control sidecar consumes only fresh exact `/usage` enqueue appends from startup-pinned transcript FDs and returns the latest private statusLine `rate_limits` snapshot as bold percentages and compact micro-bars without an extra Claude process, model turn, or second Telegram poller; while structured quota state is active, later ordinary messages receive the same quoted reset notice instead of silent `👀`;
