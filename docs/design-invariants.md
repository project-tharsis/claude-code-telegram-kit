...
- Completed assistant text is captured directly from bounded native `MessageDisplay` events. A later PreToolUse commits each completed buffered message as interim commentary, closes the preceding progress segment, sends the commentary silently and at most once, then opens the next segment. MessageDisplay capture performs no network I/O; model prose never enters a tool-progress bubble, and buffered text with no later tool is discarded at Stop and delivered only as final.
