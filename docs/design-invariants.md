...
- PostToolUse and PostToolUseFailure mark each tool ID as running, completed, or failed. One turn owns an ordered sequence of wire-bounded tool segments. Every normal step remains visible within its segment, and folding is reserved for Telegram's hard message limit rather than an arbitrary step count.
- A completed assistant text block becomes interim commentary only when a later PreToolUse event is proven by the exact bound transcript. For each trusted PreToolUse, the renderer performs one immediate read plus one bounded 35ms catch-up read for rows flushed just afterward; overlapping hooks catch up concurrently while segment writes remain ordered. It then fails open and records the tool, closes the preceding progress segment, sends the bounded redacted commentary silently and at most once, and opens the next segment. Model prose never enters a tool-progress bubble, and text with no later tool remains canonical final only.
- `MessageDisplay` is a local screen-display hook, not a Channel response hook. Live Claude Code 2.1.240 official-Telegram turns emitted normal UserPromptSubmit, Pre/PostToolUse, and Stop hooks but no MessageDisplay events for transcript-proven assistant prose. Channel commentary therefore uses the exact secure transcript append; do not replace it with display-layer events without a fresh live Channel receipt.
- Tool-use deduplication is turn-global across segments. A sealed progress bubble is immutable, and only the current segment may be edited, preserving Telegram chronology.
- Each turn selects one spinner/completion verb pair from a fixed Claude-style allowlist by stable session/prompt identity. The pair never comes from model text or tool output and remains stable across every segment in the turn.
...
- Presentation is fail-open for the agent: hook, send, edit, throttle, and restart failures never block tool execution or the final reply. A commentary boundary may wait only on ephemeral 500-millisecond Telegram attempts: at most one already in-flight progress update, one completed-segment seal, and one commentary send. Timeout aborts that request, preserves its terminal uncertainty, and releases PreToolUse; canonical final delivery keeps its separate timeout.
- Each segment owns at most one replacement progress bubble; unknown sends are never retried and 429 ends that segment's disclosure. Commentary delivery has no reaction side effects, and delivered, rejected, and uncertain outcomes are all terminal for the reserved commentary block.
...
- Automatic naming runs as a service-user command hook, not inside the MCP child, so it inherits the same Claude authentication source without copying credentials into `.mcp.json`. Stop is the primary trigger; `/resume` (and legacy `/sessions`) plus `/reset` are backstops. Failure is display-only and never blocks reply delivery or session control.
...
- The unprivileged control MCP communicates only with a mode-`0600`, non-root service-user-owned Unix socket. Broker Protocol v2 accepts `capabilities|reset|resume|model`; the root broker rejects UID 0, checks `SO_PEERCRED`, limits mutations to 12 per minute and four concurrent helper jobs, and derives every unit, request ID, helper/config path, and fixed no-shell systemd argv.
