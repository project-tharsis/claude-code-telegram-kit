...
- The unprivileged control MCP communicates only with a mode-`0600`, service-user-owned Unix socket. Broker Protocol v1 accepts `capabilities|reset|resume|model`; the root broker checks `SO_PEERCRED` and derives every unit, request ID, helper/config path, and fixed no-shell systemd argv.
