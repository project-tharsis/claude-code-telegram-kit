...
- `/sessions` and `/resume N` require a current-turn capability created from an exact Telegram command.
- A numbered list is an atomic, private, ten-minute snapshot. Later session activity never repoints a visible index.
- The model can supply an index but never a session UUID, transcript path, helper path, service, unit, or command.
- Resume stays within one root-configured workspace, is approval-gated, revalidates at both privilege levels, and rolls back the previous session on failed health checks.
- Official Telegram plugin 0.0.7 does not include `forward_origin` in Channel metadata. An allowlisted user forwarding text whose entire body is an exact command is indistinguishable from typing it; mutating reset/resume commands retain Telegram approval as the safety boundary.

## Reset

- Reset acknowledgement is sent by the control plane, quotes the triggering `/reset`, and is not generated by the model.
