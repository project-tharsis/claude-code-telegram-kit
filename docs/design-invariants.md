...
- `verbose` may expose ordinary VM commands, paths, queries, and URLs by user choice. Credential-shaped values are always replaced with fixed redaction markers before Telegram delivery; ordinary previews are then bounded to 40 characters (`verbose`) or 28 (`all`), while bounded `Skill` names may use up to 128 characters. Fixed emoji and friendly verbs render ordinary previews inline; only `Bash` commands use a one-line Telegram `shell` block. Every dynamic value is HTML-escaped before delivery.
- PostToolUse and PostToolUseFailure mark each tool ID as running, completed, or failed. One turn owns one wire-bounded bubble: every normal step remains visible, and folding is reserved for Telegram's hard message limit rather than an arbitrary step count.
