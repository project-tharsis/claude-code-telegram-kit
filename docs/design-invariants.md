...
- Disclosure accepts turn/tool/agent IDs plus an explicit allowlist of bounded preview strings (`command`, `file_path`, `path`, `pattern`, `query`, `url`, `skill`, `description`). It never accepts the raw `tool_input` object or any tool output.
