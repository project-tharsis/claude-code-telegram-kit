...
- Under active structured quota, `/usage` never renders local snapshot percentages. Optional `CLAUDE_OAUTH_USAGE_ENABLED=true` permits one read-only on-demand OAuth usage GET with a 120-second success cache and five-minute failure/rejection cooldown; credentials are owner-bound mode-0600, never refreshed or written, and failure/429 may fall back only to a statusLine snapshot no more than 15 minutes old with an explicit capture time; older snapshots show reached/reset when known or `Live usage unavailable`.
...
- Session-title background-task scanning accepts dual terminal identifiers only when the task alias resolves to the same canonical tool route; mismatches are fail-closed and leave the fork pending.
