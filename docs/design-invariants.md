...
- Automatic naming runs as a service-user command hook, not inside the MCP child, so it inherits the same Claude authentication source without copying credentials into `.mcp.json`. Stop is the primary trigger; `/sessions` and `/reset` are backstops. Failure is display-only and never blocks reply delivery or session control.
