...
- Deployments that explicitly select `CLAUDE_CODE_AUTH_PREFLIGHT=interactive-login` run one bounded, no-shell `claude auth status --text` preflight before sustained typing or model execution. Only the exact `Login: Expired` status sends one replay-deduplicated quoted explanation and blocks; alternate credential environments and unknown probe failures fail open. No periodic auth polling or token policy belongs to the harness.
