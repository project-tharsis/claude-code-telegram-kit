...
- A completed assistant text block becomes interim commentary only when a later PreToolUse event is proven by the exact bound transcript. The renderer closes the preceding progress segment, sends the bounded redacted commentary silently and at most once, then opens the next segment. Model prose never enters a tool-progress bubble, and text with no later tool remains canonical final only.
