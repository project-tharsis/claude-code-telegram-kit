...
When the trusted transcript proves that completed assistant text precedes a later tool call, the renderer closes the current progress segment, sends that bounded redacted commentary as a separate silent MarkdownV2 message, and opens a new segment before releasing the PreToolUse hook. Ephemeral progress send/edit and commentary requests each have a 500-millisecond abort budget instead of inheriting canonical final delivery's timeout; a boundary can wait on at most one already in-flight progress update, one seal, and one commentary send. Commentary never finalizes the inbound reaction and every transport outcome is terminal, so an uncertain send is never retried. Text without a later tool remains final-only. `Stop` drains the final segment before exactly-once canonical delivery; `StopFailure` marks unfinished steps failed without sending assistant text.
