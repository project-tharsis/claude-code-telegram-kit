...
`examples/telegram-settings.json` wires `UserPromptSubmit`, `MessageDisplay`, `PreToolUse`, `PostToolUse`, `PostToolUseFailure`, `Stop`, and `StopFailure` to six internal MCP tools. The internal tools are denied to the model but remain callable by Claude Code's supported `mcp_tool` hook handler.

One direct Telegram turn owns an ordered sequence of silent progress segments. `TELEGRAM_TOOL_DISCLOSURE_MODE` selects `safe`, `all`, or `verbose`; `verbose` shows at most 40 preview characters and `all` at most 28 while hard-redacting credential values. Telegram HTML renders the header/tool label in bold and the escaped preview as monospace code. Tool outputs never enter disclosure. PostToolUse events render running (`…`), completed (`✓`), and failed (`✕`) state. Updates debounce and deduplicate `tool_use_id` turn-wide.

When a bounded native `MessageDisplay` stream marks an assistant message complete, the renderer holds it in memory until the next tool call. A later PreToolUse commits that completed message as a separate silent MarkdownV2 commentary message, closes the current progress segment, and opens a new segment before releasing the tool. MessageDisplay capture performs no network I/O and returns immediately. Ephemeral progress send/edit and commentary requests each have a 500-millisecond abort budget instead of inheriting canonical final delivery's timeout; a boundary can wait on at most one already in-flight progress update, one seal, and one commentary send. Commentary never finalizes the inbound reaction and every transport outcome is terminal, so an uncertain send is never retried. Buffered text with no later tool is discarded at Stop and delivered only through the canonical final path.
