...
function updateBackgroundTaskState(
  ...
  for (const value of values) {
    ...
    if (direct !== undefined && notification.taskId !== undefined && aliased !== direct) continue;
    const toolUseId = direct ?? aliased;
    if (toolUseId !== undefined) taskIds.delete(toolUseId);
    if (notification.taskId !== undefined && aliased === toolUseId) taskAliases.delete(notification.taskId);
  }
  return false;
}
