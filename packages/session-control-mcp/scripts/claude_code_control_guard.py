...
and blocks direct `/usage`, `/sessions`, `/reset`, and `/resume` namespaces before the LLM. If the MCP is
...
CONTROL_NAMESPACE = re.compile(r"^/(?:usage|sessions|reset|resume)")
...
def should_block(payload: Any) -> bool:
    ...
    return body is not None and CONTROL_NAMESPACE.match(body) is not None
