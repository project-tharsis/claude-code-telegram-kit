...
When a subagent is still active after the parent final, the renderer opens one quoted `Background work` bubble. Official subagent lifecycle and nested tool events edit that bubble with bounded agent identity and the latest verified action. Agent type labels come from Claude Code lifecycle metadata. When all children stop, the bubble stays `Finalizing…` until the parent task terminal event. Structured terminals render `completed` as `Done`, `failed` as `Failed`, and `killed` as `Stopped`; a fixed `PostToolUse:TaskStop` hook covers cancellation notifications absorbed into a busy direct turn. There is no interval, transcript polling, extra model turn, fabricated percentage, or raw subagent output. Completion still returns automatically through the separately bound final-delivery path; if that authority-bound completion turn forks another verified subagent, the same bounded contract continues for the descendant phase without exposing the completion turn's own foreground tools.

## Operational failures

Runtime failures do not depend on Claude generating a reply. Typed `StopFailure.error` is primary. For background quota exhaustion on Claude Code 2.1.241, a failed task may instead produce only a structured API-error transcript row, so the exact consumed task route gets a five-second bounded append watcher. The renderer sends one quoted fixed notice, includes a validated reset time when available, and can fill missing typed rate-limit reset metadata only from a secure private `statusLine.rate_limits` snapshot that proves an exhausted window. If several windows are exhausted, it reports the latest reset; otherwise it keeps the generic text. Parallel failures are deduplicated, provider prose is never forwarded, and unknown sends are never retried. While a structured quota window is active, the attested queue sidecar also acknowledges later ordinary messages with the same quoted reset notice instead of leaving them at `👀`. `/usage` hides local snapshot percentages in that state. If explicitly enabled, one on-demand read-only OAuth request may add realtime bars; on failure, only a statusLine snapshot no more than 15 minutes old is shown with its capture time, otherwise reached/reset or `Live usage unavailable` remains.
